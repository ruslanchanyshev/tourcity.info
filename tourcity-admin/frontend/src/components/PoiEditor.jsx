...
const SERVICE_DISCOUNT_CODES = [
  { code: '200', text: 'Скидка на первое посещение' },
  { code: '201', text: 'Первый урок бесплатно' },
  { code: '202', text: 'Скидка на групповые занятия' },
  { code: '203', text: 'Бесплатная доставка' },
  { code: '204', text: 'Приведи друга (скидка обоим)' },
  { code: '205', text: 'Раннее бронирование' },
  { code: '206', text: 'Скидка за отзыв с фото' },
  { code: '211', text: 'Скидка за отметку в сторис' },
  { code: '212', text: 'Консультация / диагностика бесплатно' },
  { code: '213', text: 'Счастливые часы в будни' },
  { code: '207', text: 'Подарок к заказу' },
  { code: '208', text: 'Пакет услуг (от 3-х)' },
  { code: '210', text: 'Специальное предложение' },
];
...
export default function PoiEditor({ mode = 'places', poi, initialDraft, onDraftChange, onSave, onCancel }) {
  ...
  const discountCodes = mode === 'services' ? SERVICE_DISCOUNT_CODES : DISCOUNT_CODES;
  const discountField = mode === 'services' ? 'info_discount' : 'ext_4';
  const sizeField = mode === 'services' ? 'size_discount' : 'ext_2';
  const expField = mode === 'services' ? 'exp_discount' : 'ext_3';

  const selectedDiscountItem = discountCodes.find(c => c.code === data[discountField]);
                  ...
                  <>
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
                      <div style={{ position:'relative' }}>
                        ...
                        <input className="premium-input" placeholder="Ник или ссылка" value={data.inst || ''} onChange={e => handleChange('inst', e.target.value)} />
                      </div>
                      <div style={{ position:'relative' }}>
                        ...
                        <input className="premium-input" placeholder="Ник (без @)" value={data.tg || ''} onChange={e => handleChange('tg', e.target.value)} />
                      </div>
                    </div>
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
                      <div style={{ position:'relative' }}>
                        <span style={{ position:'absolute', left:12, top:-8, fontSize:9, background:'var(--bg-card)', padding:'0 4px', color:'var(--text-muted)' }}>WHATSAPP</span>
                        <input className="premium-input" placeholder="Номер WhatsApp" value={data.wtsp || ''} onChange={e => handleChange('wtsp', e.target.value)} />
                      </div>
                      <div style={{ position:'relative' }}>
                        <span style={{ position:'absolute', left:12, top:-8, fontSize:9, background:'var(--bg-card)', padding:'0 4px', color:'var(--text-muted)' }}>НОМЕР ДЛЯ ЗВОНКА</span>
                        <input className="premium-input" placeholder="Телефон для связи" value={data.call || ''} onChange={e => handleChange('call', e.target.value)} />
                      </div>
                    </div>
                    <div style={{ position:'relative' }}>
                      ...
                      <input className="premium-input" placeholder="https://..." value={data.site || ''} onChange={e => handleChange('site', e.target.value)} />
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
                      <div style={{ position:'relative' }}>
                        <span style={{ position:'absolute', left:12, top:-8, fontSize:9, background:'var(--bg-card)', padding:'0 4px', color:'var(--text-muted)' }}>INSTAGRAM</span>
                        <input className="premium-input" placeholder="Instagram URL" value={data.inst || ''} onChange={e => handleChange('inst', e.target.value)} />
                      </div>
                      <div style={{ position:'relative' }}>
                        <span style={{ position:'absolute', left:12, top:-8, fontSize:9, background:'var(--bg-card)', padding:'0 4px', color:'var(--text-muted)' }}>TELEGRAM</span>
                        <input className="premium-input" placeholder="Telegram" value={data.tg || ''} onChange={e => handleChange('tg', e.target.value)} />
                      </div>
                    </div>
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
                      <div style={{ position:'relative' }}>
                        <span style={{ position:'absolute', left:12, top:-8, fontSize:9, background:'var(--bg-card)', padding:'0 4px', color:'var(--text-muted)' }}>WHATSAPP</span>
                        <input className="premium-input" placeholder="WhatsApp" value={data.wtsp || ''} onChange={e => handleChange('wtsp', e.target.value)} />
                      </div>
                      <div style={{ position:'relative' }}>
                        <span style={{ position:'absolute', left:12, top:-8, fontSize:9, background:'var(--bg-card)', padding:'0 4px', color:'var(--text-muted)' }}>НОМЕР ДЛЯ ЗВОНКА</span>
                        <input className="premium-input" placeholder="Телефон" value={data.call || ''} onChange={e => handleChange('call', e.target.value)} />
                      </div>
                    </div>
                    <div style={{ position:'relative' }}>
                      <span style={{ position:'absolute', left:12, top:-8, fontSize:9, background:'var(--bg-card)', padding:'0 4px', color:'var(--text-muted)' }}>САЙТ / ПОРТФОЛИО</span>
                      <input className="premium-input" placeholder="Веб-сайт" value={data.site || ''} onChange={e => handleChange('site', e.target.value)} />
                    </div>
                  </>
                )}
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
                ...
                <div>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
                    <span style={{ fontSize:10, fontWeight:800, textTransform:'uppercase', letterSpacing:'0.15em', color:'var(--text-muted)' }}>Описание</span>
                    <span style={{ fontSize:10, fontWeight:800, color: (data[`desc_${selectedLang}`]?.length || 0) > 200 ? '#ff4444' : 'var(--text-muted)' }}>
                      {(data[`desc_${selectedLang}`]?.length || 0)} / 200
                    </span>
                  </div>
                  <textarea 
                    ...
                    style={{ 
                      ...
                      lineHeight:1.8, 
                      fontSize:14,
                      border: (data[`desc_${selectedLang}`]?.length || 0) > 200 ? '1px solid rgba(255,68,68,0.4)' : ''
                    }} 
                    value={data[`desc_${selectedLang}`] || ''} 
                    onChange={e => handleChange(`desc_${selectedLang}`, e.target.value)} 
                  />
                </div>
              </div>
            </div>
            ...
            <div className="editor-card">
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:24 }}>
                <div>
                  ...
                  <input className="premium-input" style={{ textAlign:'center', fontSize:22, fontWeight:900, color:'var(--accent-gold)' }} placeholder="0" value={data[sizeField] || ''} onChange={e => handleChange(sizeField, e.target.value)} />
                </div>
                <div>
                  ...
                  <input className="premium-input" style={{ textAlign:'center', fontWeight:700 }} placeholder="31.12.2024" value={data[expField] || ''} onChange={e => handleChange(expField, e.target.value)} />
                </div>
              </div>
                ...
                {showDiscountSelector && (
                  <div style={{ background:'rgba(0,0,0,0.5)', border:'1px solid rgba(212,161,23,0.15)', borderRadius:16, padding:12, maxHeight:280, overflowY:'auto' }} className="animate-fade">
                    {discountCodes.map(item => {
                      const isSelected = data[discountField] === item.code;
                      return (
                        <div
                          key={item.code}
                          onClick={() => { handleChange(discountField, item.code); setShowDiscountSelector(false); }}
                          style={{
            ...
            <div className="editor-card">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  ...
                  <input className="premium-input" placeholder="Например: Марафон по бегу" value={data.event || ''} onChange={e => handleChange('event', e.target.value)} />
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <div>
                    ...
                    <input 
                      className="premium-input" 
                      type="date"
                      value={(data.day_event && typeof data.day_event === 'string' && data.day_event.includes('.')) ? data.day_event.split('.').reverse().join('-') : (data.day_event || '')} 
                      onChange={e => {
                        const val = e.target.value;
                        if (!val) {
                          handleChange('day_event', '');
                        } else {
                          const [y, m, d] = val.split('-');
                          handleChange('day_event', `${d}.${m}.${y}`);
                        }
                      }}
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      <select 
                        ...
                        value={(data.time_event || '09:00').split(':')[0]} 
                        onChange={e => {
                          const mins = (data.time_event || '09:00').split(':')[1] || '00';
                          handleChange('time_event', `${e.target.value}:${mins}`);
                        }}
                      >
                      ...
                      <select 
                        className="premium-input" 
                        style={{ flex: 1, padding: '10px' }}
                        value={(data.time_event || '09:00').split(':')[1] || '00'} 
                        onChange={e => {
                          const hours = (data.time_event || '09:00').split(':')[0] || '09';
                          handleChange('time_event', `${hours}:${e.target.value}`);
                        }}
                      >
                ...
                <div>
                  ...
                  <input className="premium-input" placeholder="Парк им. Горького" value={data.place_event || ''} onChange={e => handleChange('place_event', e.target.value)} />
                </div>
                ...
                <div>
                  ...
                  <input className="premium-input" placeholder="Предварительная запись" value={data.information || ''} onChange={e => handleChange('information', e.target.value)} />
                </div>
              </div>
            </div>
          </div>
